...
bool FBXLoader::LoadTextureFromFile(const FbxString & pFilePath, unsigned int & pTextureObject)
{
	if (pFilePath.Right(3).Upper() == "TGA")
	{
		...
		if (tga_read(&lTGAImage, pFilePath.Buffer()) == TGA_NOERR)
		{
			...
			//glTexImage2D(GL_TEXTURE_2D, 0, 3, lTGAImage.width, lTGAImage.height, 0, GL_BGR,GL_UNSIGNED_BYTE, lTGAImage.image_data);
			glTexImage2D(GL_TEXTURE_2D, 0, 3, lTGAImage.width, lTGAImage.height, 0, GL_RGB,GL_UNSIGNED_BYTE, lTGAImage.image_data);
...
void FBXLoader::GlDrawLimbNode(FbxAMatrix& pGlobalBasePosition, FbxAMatrix& pGlobalEndPosition)
{
	...
	//glVertex3dv((double *)pGlobalBasePosition.GetT());
	//glVertex3dv((double *)pGlobalEndPosition.GetT());
